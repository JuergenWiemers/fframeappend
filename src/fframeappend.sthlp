...
and the value {it:k} for observations from the {it:k}th frame in the {it:framelist}. Caution: If wildcards are used in the {it:framelist} all matching frames are added in alphabetical order. {p_end}
...
If no {it:{help varlist}} is provided, all variables in frames {it: framlist} will be appended to the current frame. 
